...
class AccountsListsForm:
    def __init__(self, CC, linked_combo=None):
        '''
        ...
        Les variables sont :
            - lists       = liste des groupes de comptes
    ...
    def prepare(self,lists,lists_logos):
        self.lists = lists
        self.lists_logos = lists_logos
        ...
        if self.cb_lists.Items.ToList!=self.klist:
            ...
            self.init_lists()
        current = ""
        if not self.linked_combo is None:
            if self.linked_combo.ItemIndex!=-1: current = self.linked_combo.Items[self.linked_combo.ItemIndex]
        self.cb_lists.ItemIndex = self.cb_lists.Items.Text.split('\n').index(current)

    def update(self,Sender,x):
        if not self.linked_combo is None:
            if self.cb_lists.ItemINdex!=-1 and not linked_combo is None:
    ...
    def init_lists(self,Sender=None):
        ...
        if self.cb_lists.ItemIndex!=-1:
            name = self.cb_lists.Items[self.cb_lists.ItemIndex]
            current_list = self.lists[name]
            self.new_list.Text=name
            self.group_logo = self.lists_logos.get(name,Path + "groups\\_pas de logo_.png")
            self.picker.ToTuple()[0].Picture.LoadFromFile(self.group_logo)
        else:
            current_list=[]
            self.new_list.Text=''
    ...
    def set_list(self,Sender=None):
        ...
        self.lists_logos[name]=self.group_logo
        self.klist = self.lists.keys()
        self.klist.sort()
